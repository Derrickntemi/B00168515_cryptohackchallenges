...
class ChaCha20:
    ...
    def _quarter_round_inv(self, x, a, b, c, d):
        x[b] = rotate_inv(x[b], 7);
        x[b] ^= x[c];
        x[c] = word(x[c] - x[d])
        x[d] = rotate_inv(x[d], 8);
        x[d] ^= x[a];
        x[a] = word(x[a] - x[b])
        x[b] = rotate_inv(x[b], 12);
        x[b] ^= x[c];
        x[c] = word(x[c] - x[d])
        x[d] = rotate_inv(x[d], 16);
        x[d] ^= x[a];
        x[a] = word(x[a] - x[b])
...
if __name__ == '__main__':
    ...
    msg_enc = bytes.fromhex(
        'f3afbada8237af6e94c7d2065ee0e221a1748b8c7b11105a8cc8a1c74253611c94fe7ea6fa8a9133505772ef619f04b05d2e2b0732cc483df72ccebb09a92c211ef5a52628094f09a30fc692cb25647f')
